    ...
    int largestRectangleArea(vector<int>& heights)
    {
        ...
        */
        ...
        for (int i = 0; i < heights.size(); ++i)
        {
            ...
            {
                int prev_top = mono_stack.top();
                int h = heights[mono_stack.top()];
                mono_stack.pop();
                /**
                 * heights[i] and current heights[mono_stack.top()] are the most recent bins
                 * strictly < previous heights[mono_stack.top()] = h
                 * hence, the rectangle of height h is [current top + 1, i-1]
                 * Since the stack is monon-non-descending, heights[current_top + 1] > h
                 */
                std::cout << "i " << i << ", prev top: " << prev_top << std::endl;
